...
class MF(GeneralRecommender):
    def __init__(self, config):
        ...
        self.embed_user = nn.Embedding(config['user_num'], config['factors']).cuda()
        self.embed_item = nn.Embedding(config['item_num'], config['factors']).cuda()
...
#         return rec_ids.cpu().numpy()
    ...
    def rank(self, test_loader):
        rec_ids = torch.tensor([], device=self.device, dtype=torch.long)  # 初始化为空张量，类型为 long

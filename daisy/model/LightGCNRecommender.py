...
#             scores = torch.bmm(user_emb, item_emb).squeeze(1) # batch * cand_num
...
class LightGCN(GeneralRecommender):
    ...
    def get_norm_adj_mat(self):
        ...
        # 归一化邻接矩阵
        sum_arr = (A > 0).sum(axis=1)
        diag = np.array(sum_arr.flatten())[0] + 1e-7  # 避免除以零
        diag = np.power(diag, -0.5)
        D = sp.diags(diag)
        L = D * A * D
        ...
        row = L.row
        col = L.col
        i = torch.LongTensor(np.array([row, col])).to(self.device)  # 移动到 GPU
        data = torch.FloatTensor(L.data).to(self.device)  # 移动到 GPU
        SparseL = torch.sparse.FloatTensor(i, data, torch.Size(L.shape))

        return SparseL

    def get_ego_embeddings(self):
        ''' 获取用户和物品的嵌入并拼接成一个新的嵌入矩阵 '''
        user_embeddings = self.embed_user.weight
        item_embeddings = self.embed_item.weight
        ego_embeddings = torch.cat([user_embeddings, item_embeddings], dim=0)
...
#         return user_embedding, item_embedding
    def forward(self):
        ...
        # 进行多层图卷积
        for _ in range(self.num_layers):
            # 确保 norm_adj_matrix 和 all_embeddings 在同一个设备上
            all_embeddings = torch.sparse.mm(self.norm_adj_matrix, all_embeddings)
            embeddings_list.append(all_embeddings)
